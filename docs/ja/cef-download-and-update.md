...
展開した `<packageName>/<target>` 直下にあるアプリランチャーを起動すると、`muon-launcher` は同じディレクトリ直下へCEFをin-placeで準備し、`muon-core` もそのディレクトリをカレントディレクトリとして起動します。
...
> 注釈: ここまでの解説では、Linux debパッケージからの起動を除きます。
> Linux debパッケージでは、管理者権限で配置されるディレクトリにmuonアプリが配置され、CEFのダウンロードと配置も管理者権限が必要なディレクトリに配置されます。
> その代わり、LinuxのCEFで必要となる `cef-sandbox` が完全に有効化されます。
