...
- `muonPath`, `cefPath`, `stagePath`, `open`, `enableDebugger`, `allowInsecureLocalhost`, `exitWithServer` は `vite dev` に影響します。
  `muon run` は `muonPath`, `cefPath`, `stagePath`, `enableDebugger`, `allowInsecureLocalhost` と `build.configPath` を読み取り、 `open` と `exitWithServer` は無視します。
