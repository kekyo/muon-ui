...
case "${TARGET}" in
  linux-amd64)
    ...
    LAUNCHER_EXECUTABLE_NAME="muon-launcher"
    RUNTIME_HELPER_EXECUTABLE_NAME="muon-runtime-helper"
    PLUGIN_INSPECTOR_EXECUTABLE_NAME="muon-plugin-inspector"
    LDFLAGS_VALUE="${LDFLAGS:--static}"
    LAUNCHER_LDFLAGS_VALUE="${LAUNCHER_LDFLAGS:-}"
    PLUGIN_INSPECTOR_LDFLAGS_VALUE="${PLUGIN_INSPECTOR_LDFLAGS:-}"
    PLUGIN_INSPECTOR_LDLIBS_EXTRA="-ldl"
    ;;
  linux-armhf)
  ...
  windows-i686)
    ...
    LAUNCHER_EXECUTABLE_NAME="muon-launcher.exe"
    RUNTIME_HELPER_EXECUTABLE_NAME=""
    PLUGIN_INSPECTOR_EXECUTABLE_NAME="muon-plugin-inspector.exe"
    LDFLAGS_VALUE="${LDFLAGS:--static -static-libgcc}"
    LAUNCHER_LDFLAGS_VALUE="${LAUNCHER_LDFLAGS:-${LDFLAGS_VALUE} -mwindows}"
    PLUGIN_INSPECTOR_LDFLAGS_VALUE="${PLUGIN_INSPECTOR_LDFLAGS:-${LDFLAGS_VALUE}}"
    PLUGIN_INSPECTOR_LDLIBS_EXTRA=""
    ;;
  windows-amd64)
...
if [[ "${TARGET_NAME}" == windows-* ]]; then
  ...
  LAUNCHER_RESOURCE_SCRIPT="${OUT_DIR}/muon_launcher.rc"
  LAUNCHER_RESOURCE_OBJECTS_VALUE="${OUT_DIR}/muon_launcher_resource.o"
  write_windows_version_resource_script \
    "${LAUNCHER_RESOURCE_SCRIPT}" \
    "" \
    "muon_launcher.manifest" \
    "muon Launcher" \
    "muon-launcher" \
...
LAUNCHER_CPPFLAGS_VALUE="${LAUNCHER_CPPFLAGS_EXTRA}"
if [[ -n "${LAUNCHER_CPPFLAGS:-}" ]]; then
...
LAUNCHER_LDLIBS_VALUE="${LAUNCHER_LDLIBS_EXTRA}"
if [[ -n "${LAUNCHER_LDLIBS:-}" ]]; then
  LAUNCHER_LDLIBS_VALUE="${LAUNCHER_LDLIBS_VALUE} ${LAUNCHER_LDLIBS}"
fi
PLUGIN_INSPECTOR_LDLIBS_VALUE="${PLUGIN_INSPECTOR_LDLIBS_EXTRA}"
if [[ -n "${PLUGIN_INSPECTOR_LDLIBS:-}" ]]; then
  PLUGIN_INSPECTOR_LDLIBS_VALUE="${PLUGIN_INSPECTOR_LDLIBS_VALUE} ${PLUGIN_INSPECTOR_LDLIBS}"
fi
...
PLUGIN_INSPECTOR_TARGET_VALUE="${OUT_DIR}/${PLUGIN_INSPECTOR_EXECUTABLE_NAME}"
...
make -j -C "${SCRIPT_DIR}" -B \
  ...
  LAUNCHER_TARGET="${OUT_DIR}/${LAUNCHER_EXECUTABLE_NAME}" \
  RUNTIME_HELPER_TARGET="${RUNTIME_HELPER_TARGET_VALUE}" \
  PLUGIN_INSPECTOR_TARGET="${PLUGIN_INSPECTOR_TARGET_VALUE}" \
  VERSION_HEADER="${VERSION_HEADER}" \
  RUNTIME_INFO_HEADER="${RUNTIME_INFO_HEADER}" \
  CPPFLAGS="${CPPFLAGS_VALUE}" \
  LAUNCHER_CPPFLAGS="${LAUNCHER_CPPFLAGS_VALUE}" \
  CFLAGS="${CFLAGS_VALUE}" \
  LDFLAGS="${LDFLAGS_VALUE}" \
  LAUNCHER_LDFLAGS="${LAUNCHER_LDFLAGS_VALUE}" \
  PLUGIN_INSPECTOR_LDFLAGS="${PLUGIN_INSPECTOR_LDFLAGS_VALUE}" \
  PREPARE_WINDOWS_RESOURCE_OBJECTS="${PREPARE_WINDOWS_RESOURCE_OBJECTS_VALUE}" \
  LAUNCHER_RESOURCE_OBJECTS="${LAUNCHER_RESOURCE_OBJECTS_VALUE}" \
  LDLIBS="${LDLIBS_VALUE}" \
  LAUNCHER_LDLIBS="${LAUNCHER_LDLIBS_VALUE}" \
  PLUGIN_INSPECTOR_LDLIBS="${PLUGIN_INSPECTOR_LDLIBS_VALUE}"
