...
- `muonPath`, `cefPath`, `stagePath`, `open`, `enableDebugger`, and `exitWithServer` affect `vite dev`.
  `muon run` reads `muonPath`, `cefPath`, `stagePath`, `enableDebugger`, and `build.configPath`, and ignores `open` and `exitWithServer`.
